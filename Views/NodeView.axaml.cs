...
public partial class NodeView : UserControl
{
    ...
    protected override void OnDataContextChanged(EventArgs e)
    {
        ...
        if (DataContext is Node node) Node = node;
    }
}
...
public class StateToFillBrushConverter : IValueConverter
{
    ...
    {
        if (value is Node node) return GetBrushForState(node.State);
        if (value is NodeState state) return GetBrushForState(state);
        return GetBrushForState(NodeState.Normal);
    }

    private IBrush GetBrushForState(NodeState state) => state switch
    {
        ...
        NodeState.Path => new SolidColorBrush(Color.Parse("#1E88E5")),
        NodeState.Goal => new SolidColorBrush(Color.Parse("#FFA500")),
...
public class StateToStrokeBrushConverter : IValueConverter
{
    ...
    private IBrush GetBrushForState(NodeState state) => state switch
    {
        ...
        NodeState.Selected => new SolidColorBrush(Color.Parse("#FFA500")),
        NodeState.Path => new SolidColorBrush(Color.Parse("#1565C0")),
        NodeState.Goal => new SolidColorBrush(Color.Parse("#FFA500")),
